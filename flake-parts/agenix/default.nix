# --- flake-parts/agenix/default.nix
{
  ...
  config = {
    ...
    flake.nixosModules.security_agenix = {
      ...
      in {
        options.forge.security.agenix = with types; {
    ...
    flake.darwinModules.security_agenix = {
      config,
      lib,
      pkgs,
      system,
      ...
      in {
        ...
        imports = with inputs; [agenix.darwinModules.default];
    ...
    flake.homeModules.security_agenix = {
      ...
      in {
        options.forge.hm.security.agenix = with types; {
          enable = mkEnableOption ''
